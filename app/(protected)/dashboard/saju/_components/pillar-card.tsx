...
const OHENG_COLORS: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  mok: { bg: "bg-emerald-50 dark:bg-emerald-950/30", text: "text-emerald-700 dark:text-emerald-400", border: "border-emerald-200 dark:border-emerald-800", dot: "bg-emerald-500" },
  hwa: { bg: "bg-red-50 dark:bg-red-950/30", text: "text-red-700 dark:text-red-400", border: "border-red-200 dark:border-red-800", dot: "bg-red-500" },
  to:  { bg: "bg-amber-50 dark:bg-amber-950/30", text: "text-amber-700 dark:text-amber-400", border: "border-amber-200 dark:border-amber-800", dot: "bg-amber-500" },
  geum:{ bg: "bg-slate-50 dark:bg-slate-900/50", text: "text-slate-600 dark:text-slate-300", border: "border-slate-200 dark:border-slate-700", dot: "bg-slate-400" },
  su:  { bg: "bg-blue-50 dark:bg-blue-950/30", text: "text-blue-700 dark:text-blue-400", border: "border-blue-200 dark:border-blue-800", dot: "bg-blue-500" },
};

const OHENG_LABELS: Record<string, string> = {
  mok: "木 Wood", hwa: "火 Fire", to: "土 Earth", geum: "金 Metal", su: "水 Water",
};
...
export function PillarCard({ position, pillar, stage, className }: PillarCardProps) {
  const label = PILLAR_LABELS[position]!;
  const ganColors = OHENG_COLORS[pillar.cheongan.oheng]!;
  const jiColors = OHENG_COLORS[pillar.jiji.oheng]!;
  ...
  return (
    ...
    >
      ...
      <div className="bg-muted/40 px-3 py-2 text-center border-b">
        ...
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{label.en}</span>
      ...
      <div className={cn("flex flex-col items-center py-4 px-3 gap-1", ganColors.bg)}>
        ...
        <span className={cn("text-xs font-medium px-2 py-0.5 rounded-full border", ganColors.text, ganColors.border)}>
      ...
      <div className={cn("flex flex-col items-center py-4 px-3 gap-1", jiColors.bg)}>
        ...
        <span className={cn("text-xs font-medium px-2 py-0.5 rounded-full border", jiColors.text, jiColors.border)}>
        ...
        <>
          ...
          <div className="px-3 py-2 text-center bg-muted/20">
            ...
            <div className="text-sm font-semibold">
              {stage.stage} <span className="text-muted-foreground font-normal">({stage.hanja})</span>
            </div>
            <div className="text-[10px] text-muted-foreground leading-tight mt-0.5">{stage.description}</div>
          </div>
        </>
      )}
    </div>
