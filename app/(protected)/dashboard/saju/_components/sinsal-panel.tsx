...
const PILLAR_LABELS: Record<string, string> = {
  year: "年 Year", month: "月 Month", day: "日 Day", hour: "時 Hour",
};
...
function PowerBars({ level }: { level: string }) {
  const config = POWER_CONFIG[level] ?? POWER_CONFIG.negligible!;
  return (
      ...
      {[1, 2, 3, 4].map((i) => (
        <div
          ...
          className={cn(
            ...
            i <= config.bars
              ? level === "strong" ? "bg-emerald-500" : level === "moderate" ? "bg-amber-500" : level === "weak" ? "bg-orange-500" : "bg-muted-foreground"
              : "bg-muted",
          )}
          style={{ height: `${i * 25}%` }}
        />
...
export function SinsalPanel({ interpreted }: SinsalPanelProps) {
  const gilsin = interpreted.filter((i) => i.sinsal.type === "gilsin").sort((a, b) => b.effectivePower - a.effectivePower);
  const hyungsin = interpreted.filter((i) => i.sinsal.type === "hyungsin").sort((a, b) => b.effectivePower - a.effectivePower);
...
function SinsalItem({ item }: { item: InterpretedSinsal }) {
  ...
  const powerConfig = POWER_CONFIG[item.powerLevel] ?? POWER_CONFIG.negligible!;
    ...
    <div className={cn(
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {item.isGongmang && (
            <Badge variant="outline" className="text-[10px] px-1.5 py-0 border-orange-300 text-orange-600 shrink-0">
              공망
            </Badge>
          )}
        </div>
